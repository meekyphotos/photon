...
public class ConvertToGeoJson extends Converter<List<JSONObject>, JSONObject> {
   ...
   */
  @Override
  public JSONObject doForward(final List<JSONObject> listOfPlaces) {
   ...
   */
  @Override
  public List<JSONObject> doBackward(final JSONObject jsonObject) {
    final JSONArray features = jsonObject.getJSONArray("features");
    final List<JSONObject> returnValue = new ArrayList<JSONObject>(features.length());
    for (int i = 0; i < features.length(); i++) {
      final JSONObject aSearchHit = features.getJSONObject(i);
