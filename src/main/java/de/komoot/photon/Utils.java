...
public class Utils {
  ...
  public static XContentBuilder convert(final PhotonDoc doc, final String[] languages) throws IOException {
  ...
  private static void writeExtent(final XContentBuilder builder, final Envelope bbox) throws IOException {
    if (bbox == null) {
      return;
    }
  ...
  private static void writeName(final XContentBuilder builder, final Map<String, String> name, final String[] languages) throws IOException {
    final Map<String, String> fNames = filterNames(name, languages);
  ...
  private static void write(final XContentBuilder builder, final Map<String, String> fNames, final String name) throws IOException {
    if (fNames.isEmpty()) {
      return;
    }

    builder.startObject(name);
    for (final Map.Entry<String, String> entry : fNames.entrySet()) {
      builder.field(entry.getKey(), entry.getValue());
    }
    builder.endObject();
  }

  protected static void writeContext(final XContentBuilder builder, final Set<Map<String, String>> contexts, final String[] languages) throws IOException {
    final SetMultimap<String, String> multimap = HashMultimap.create();

    for (final Map<String, String> context : contexts) {
  ...
  private static void writeIntlNames(final XContentBuilder builder, final Map<String, String> names, final String name, final String[] languages) throws IOException {
    final Map<String, String> fNames = filterNames(names, languages);
    write(builder, fNames, name);
  }

  private static Map<String, String> filterNames(final Map<String, String> names, final String[] languages) {
    return filterNames(names, new HashMap<String, String>(), languages);
  }

  private static Map<String, String> filterNames(final Map<String, String> names, final HashMap<String, String> filteredNames, final String[] languages) {
    if (names == null) {
